...
import { getCameraList, getLiveUrl } from '@/api/camera'
...
export default function TaskPage() {
  ...
  /**
   * 申请直播流 · 拿到 flvUrl / hlsUrl 后跳摄像头页
   * 摄像头页走 <web-view> + H5 flv.js 路线 (见 pages/camera/index.tsx 注释)
   */
  async function openCameraLive(cam: Camera) {
    // M4 · ref 锁 + state 锁双道防并发
    if (openingLiveRef.current) return
    openingLiveRef.current = true
    setOpeningLive(cam.cameraId)
    try {
      const live = await getLiveUrl(cam.cameraId)
      const flvUrl = live.flvUrl || ''
      const hlsUrl = live.hlsUrl || ''
      if (!flvUrl && !hlsUrl) {
        Taro.showToast({ title: '未获取到直播地址', icon: 'none' })
        return
      }
      ...
      Taro.showToast({
        title: e instanceof Error ? e.message : '获取直播地址失败',
        icon: 'none',
      })
    } finally {
      setOpeningLive(null)
      ...
      {cameras.length > 0 ? (
        <View className='task-section'>
            ...
            {cameras.map((cam) => {
              ...
              return (
                <View
                  ...
                  onClick={() => (online ? openCameraLive(cam) : null)}
                >
          ...
          <Text className='task-hint'>
            · 直播走 HTTPS WebView + H5 播放器 (flv.js / hls.js), 需已备案业务域名
          </Text>
        </View>
      ) : null}
