 ...
 *  · 过滤回环 / 链路本地 / VMware / VirtualBox / WSL 虚拟网卡
 *  · 优先返回第一个真实物理网卡的 IPv4
...
function detectLanIp(): string {
  ...
  for (const [name, addrs] of Object.entries(ifaces)) {
    if (!addrs) continue
    // 过滤明显的虚拟网卡 (名字包含 VMware / VirtualBox / WSL / vEthernet / Loopback)
    if (/vmware|virtualbox|wsl|vethernet|loopback|docker/i.test(name)) continue
