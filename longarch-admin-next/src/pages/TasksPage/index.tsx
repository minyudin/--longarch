...
export default function TasksPage() {
  ...
  return (
    ...
    >
      {/* Filter */}
      <section className="folio-page__section flex flex-wrap items-end gap-3" data-testid="tasks-filter">
        <div className="flex flex-col gap-2">
          <Label htmlFor="tasks-filter-status">STATUS</Label>
          <Select
            value={query.taskStatus || ALL}
            onValueChange={(v) =>
              setQuery((q) => ({ ...q, taskStatus: v === ALL ? '' : v, pageNo: 1 }))
            }
          >
            <SelectTrigger id="tasks-filter-status" className="w-[200px]">
              ...
              {list.map((t) => {
                ...
                return (
                  <TableRow key={t.taskId}>
                    ...
                    <TableCell className="font-folio text-ink-soft">{String(t.priority ?? '—')}</TableCell>
                    <TableCell><Badge tone={meta.tone}>{meta.cn}</Badge></TableCell>
