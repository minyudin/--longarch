  ...
  return (
    ...
    >
      ...
      <section className="folio-page__section" data-testid="dashboard-kpi">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 border border-line bg-paper-light">
          {kpi.map((s, i) => (
            <Link
              key={s.labelEn}
              to={s.href}
            ...
            >
              <div className="dashboard-kpi__value font-serif text-[44px] leading-none text-ink tracking-tight">
                {s.value}
              </div>
      ...
      <section className="folio-page__section" data-testid="dashboard-alerts">
        <h4 className="folio-page__section-title flex items-center justify-between">
          ...
          <span className="font-folio text-[10px] text-ink-faint tracking-[0.2em]">
            {totalAlerts === 0 ? 'ALL CLEAR' : `${totalAlerts} ITEMS`}
          </span>
        </h4>
...
function AlertCard({ label, labelEn, count, href, emptyText, children }: AlertCardProps) {
  const empty = count === 0
  return (
    <div className="border border-line bg-paper-light flex flex-col">
      <div className="flex items-baseline justify-between px-4 py-3 border-b border-line-soft">
        <div className="flex items-baseline gap-2">
          <span className="font-serif text-[24px] leading-none text-ink tracking-tight">
            {count}
          </span>
      ...
      <div className="flex-1 min-h-[160px]">
        {empty ? (
          <div className="py-10 text-center font-sans text-[12px] text-ink-faint">
            {emptyText}
          </div>
        ) : (
